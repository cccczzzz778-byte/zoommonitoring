...
async function route(req,res){
  ...
  if(url.pathname==='/api/oauth/start'&&req.method==='POST'){
    ...
    return json(res,200,{authorizationUrl:auth.toString(),redirectUri:redirectUri()},{'set-cookie':sidCookie(sid)});
  }
  if(url.pathname==='/oauth/zoom/callback'&&req.method==='GET'){
    ...
    if(err)return send(res,400,`Zoom authorization failed: ${err}`,{'content-type':'text/plain; charset=utf-8'});
    if(!code||!state||!p||Date.now()-p.createdAt>10*60*1000)return send(res,400,'OAuth state/code invalid yoki eskirgan',{'content-type':'text/plain; charset=utf-8'});
    ...
    try{
      ...
      return send(res,302,'',{'location':`/?zoom_auth=ok${p.meeting?`&meeting=${encodeURIComponent(p.meeting)}`:''}`,'set-cookie':sidCookie(p.sid)});
    }catch(e){return send(res,500,`Zoom token exchange xatosi: ${e.message}`,{'content-type':'text/plain; charset=utf-8'})}
  }
  ...
  if(url.pathname==='/api/oauth/logout'&&req.method==='POST'){
    if(!monitorAuth(req))return json(res,401,{error:'Monitor kaliti noto‘g‘ri'});const sid=cookieMap(req).zm_sid;if(sid)oauthSessions.delete(sid);return json(res,200,{ok:true},{'set-cookie':'zm_sid=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0'});
  }
  return proxy(req,res);
}

http.createServer((req,res)=>route(req,res).catch(e=>json(res,500,{error:e.message}))).listen(PORT,'0.0.0.0',()=>console.log(`Zoom Monitor v4 gateway: ${PORT} -> core ${CORE_PORT}`));
